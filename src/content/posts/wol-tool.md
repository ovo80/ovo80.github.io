...
description: "一款 JavaFX 桌面应用从想法到发布的全过程：技术选型、核心实现、工程细节与踩坑记录。"
image: ""
...
于是我用 Java 17 和 JavaFX 20.0.2 开发了一款 Windows 桌面应用「WOL 唤醒工具」，代码已开源至 [GitHub](https://github.com/ovo80/WOL)。本文记录该项目的技术选型、核心实现与踩坑过程。

## 技术选型

选择 Java + JavaFX 而非 Web 方案，主要考虑两点：

1. **使用场景简单**：工具只做一件事——向指定地址发送魔术包，不需要浏览器、不需要服务端，一个轻量桌面应用即可满足。
2. **部署成本低**：通过 jpackage 打包成自包含目录或 MSI 安装包后，目标机器免装 Java 运行时，直接双击运行。

版本方面锁定了 Java 17 + JavaFX 20.0.2 的组合，原因很实际：JavaFX 21 以上版本要求 JDK 21+，而本机环境是 JDK 17。
